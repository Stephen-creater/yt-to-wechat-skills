...
const scriptPath = path.join(repoRoot, "skills", "ds-imagine", "scripts", "build-batch.ts");
...
async function makeFixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "ds-imagine-build-batch-"));
...
test("build-batch omits default model so ds-imagine can resolve env or EXTEND defaults", async () => {
