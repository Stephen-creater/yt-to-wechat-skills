...
description: First-time setup and default model selection flow for ds-imagine
---
...
options:
  - label: "Project (Recommended)"
    description: ".ds-skills/ (this project only)"
  - label: "User"
    description: "~/.ds-skills/ (all projects)"
```
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.ds-skills/ds-imagine/EXTEND.md` | Current project |
| User | `$HOME/.ds-skills/ds-imagine/EXTEND.md` | All projects |
...
- In `ds-imagine`, Azure `--model` / `default_model.azure` should be the Azure deployment name, not just the underlying model family.
...
- In `ds-imagine`, `quality` is a compatibility preset. It is not a native DashScope parameter.
...
- In `ds-imagine`, Z.AI currently exposes text-to-image only; reference images are not wired for this provider.
...
options:
  - label: "google/nano-banana-2 (Recommended)"
    description: "Current default for general Replicate image generation in ds-imagine"
