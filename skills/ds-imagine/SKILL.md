---
name: ds-imagine
description: "使用本机配置的图像 API 生成或编辑图片；用于指定供应商、批量任务或 ds 工作流。"
version: 1.58.0
metadata:
  requires:
    anyBins:
...
| Path | Scope |
|------|-------|
| `.ds-skills/ds-imagine/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/ds-skills/ds-imagine/EXTEND.md` | XDG |
| `$HOME/.ds-skills/ds-imagine/EXTEND.md` | User home |
...
Legacy compatibility: if `.ds-skills/ds-image-gen/EXTEND.md` exists and the new path doesn't, the runtime renames it to `ds-imagine`. If both exist, the runtime leaves them alone and uses the new path.
...
| `DS_IMAGE_GEN_<PROVIDER>_CONCURRENCY` | Per-provider concurrency (e.g., `DS_IMAGE_GEN_REPLICATE_CONCURRENCY`) |
| `DS_IMAGE_GEN_<PROVIDER>_START_INTERVAL_MS` | Per-provider start-gap |

**Load priority**: CLI args > EXTEND.md > env vars > `<cwd>/.ds-skills/.env` > `~/.ds-skills/.env`
